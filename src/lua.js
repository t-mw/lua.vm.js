...
Lua.cfuncs = {
	"__gc": emscripten.Runtime.addFunction(function(L){
		...
		var refs = Lua.refs[getmain(L)];
		var ob = refs.get(id);
		refs.delete(id);
		Lua.refsToIdx[getmain(L)].delete(ob);
		return 0;
	}),
	"__index": emscripten.Runtime.addFunction(function(L){
		...
		var box = L.checkudata(1, "_PROXY_MT");
		var id = emscripten.getValue(box, "double");
		var ob = Lua.refs[getmain(L)].get(id);
		var k = L.lua_to_js(2);
		try {
			var res = ob[k];
		} catch (e) {
...
Lua.State = function (_L) {
	if (_L) {
		...
		var refs = new Map();
		var refsToIdx = new Map();
		refs.i = 0;
		Lua.refs[this._L] = refs;
		Lua.refsToIdx[this._L] = refsToIdx;
...
Lua.State.prototype.lua_to_js = function(i) {
	switch(this.type(i)) {
		...
		case 7: // LUA_TUSERDATA
			...
			if (box !== /* NULL */ 0) {
				var id = emscripten.getValue(box, "double");
				return Lua.refs[getmain(this)].get(id);
			}
			/* fall through */
...
Lua.State.prototype.pushjs = function(ob) {
	var refsToIdx = Lua.refsToIdx[getmain(this)];
	var i = refsToIdx.get(ob);
